...
begin
  ...
  if auth.uid() is null or auth.uid()::text <> p_user_id then
    raise exception 'try_match user_id must match authenticated user'
      using errcode = '42501';
  end if;
...
revoke execute on function public.try_match(text, text, numeric) from public, anon;
grant execute on function public.try_match(text, text, numeric) to authenticated;
