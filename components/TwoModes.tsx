...
const modes = [
  {
    ...
    href: "/blind-echo",
    accent: "from-wax-burgundy/20 to-cj-purple-card",
  },
  {
    title: "Echo Roulette",
    tag: "Casual",
    ...
    href: "/echo-roulette",
    accent: "from-signal-teal/15 to-cj-purple-card",
  },
];
      ...
      <div className="mx-auto max-w-6xl">
        ...
        <div className="mt-8 grid gap-4 sm:mt-12 sm:gap-6 md:grid-cols-2">
          {modes.map((mode) => (
            <div
              key={mode.title}
              className={`cj-jukebox-panel flex flex-col bg-gradient-to-br ${mode.accent}`}
            >
              <div className="mb-4 flex items-center justify-between">
                <CjIcon src={mode.icon} alt="" size={32} />
                <span className="cj-badge text-[10px]">{mode.tag}</span>
              </div>
              <h3 className="font-headline text-2xl uppercase text-cj-gold sm:text-3xl">
                {mode.title}
              </h3>
