  ...
  return (
    <VinylCard showDisc className="cj-auth-card w-full max-w-md !p-5 sm:!p-8">
      ...
      <div className="space-y-2.5">
        {OAUTH_BUTTONS.map(({ id, label }) => (
          <button
            ...
            className="flex min-h-11 w-full items-center justify-center gap-2.5 rounded-full border border-cj-gold-border px-4 py-2.5 text-sm leading-snug text-cj-gold transition-opacity hover:opacity-80 disabled:opacity-50 sm:gap-3 sm:text-base"
      ...
      <div className="my-5 flex items-center gap-3 sm:my-6 sm:gap-4">
        ...
        <span className="shrink-0 text-xs uppercase tracking-widest text-cj-gold-muted">Or use email</span>
        <div className="h-px flex-1 bg-cj-gold-border" />
      </div>

      {displayError && (
        <p className="cj-auth-error" role="alert">
          {displayError}
        </p>
      )}
